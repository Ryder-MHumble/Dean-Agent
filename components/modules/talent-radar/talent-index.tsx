...
import type { TalentEntry } from "@/lib/types/talent-radar";
import { mockTalents } from "@/lib/mock-data/talent-radar";

export default function TalentIndex() {
  const [selectedTalent, setSelectedTalent] = useState<TalentEntry | null>(
    null,
  );
  const topTalentCount = mockTalents.filter(
    (t) => t.influenceScore >= 95,
  ).length;
  const activeCoopCount = mockTalents.filter(
    (t) => t.cooperationStatus === "active",
  ).length;
                        ...
                        <Badge
                          ...
                          className={cn("text-[10px] w-fit", {
                            "border-green-200 bg-green-50 text-green-700":
                              talent.cooperationStatus === "active",
                            "border-blue-200 bg-blue-50 text-blue-700":
                              talent.cooperationStatus === "potential",
                            "border-gray-200 bg-gray-50 text-gray-600":
                              talent.cooperationStatus === "none",
                          })}
                        >
            ...
            <CardContent className="p-4">
              ...
              <div className="space-y-2 mb-4">
                ...
                <div className="flex items-start gap-2 text-xs">
                  ...
                  <span className="text-slate-300">
                    通过AI for Science方向对接DeepMind团队
                  </span>
                </div>
                <div className="flex items-start gap-2 text-xs">
                  <div className="h-1.5 w-1.5 rounded-full bg-amber-400 mt-1.5 shrink-0" />
                  <span className="text-slate-300">
                    筹备国际AI学术论坛，邀请LeCun等大师出席
                  </span>
                </div>
              </div>
              <Button
          ...
          {selectedTalent && (
            <>
              <SheetHeader>
                <SheetTitle className="text-lg flex items-center gap-2">
                  ...
                  <Badge
                    ...
                    className={cn("text-[10px]", {
                      "border-green-200 bg-green-50 text-green-700":
                        selectedTalent.cooperationStatus === "active",
                      "border-blue-200 bg-blue-50 text-blue-700":
                        selectedTalent.cooperationStatus === "potential",
                      "border-gray-200 bg-gray-50 text-gray-600":
                        selectedTalent.cooperationStatus === "none",
                    })}
                  >
                ...
                <SheetDescription>
                  {selectedTalent.institution} &middot;{" "}
                  {selectedTalent.discipline} &middot; H指数{" "}
                  {selectedTalent.hIndex}
                </SheetDescription>
              </SheetHeader>
              <div className="mt-6 space-y-4">
                ...
                <div className="flex gap-2 pt-4">
                  ...
                  <Button
                    variant="outline"
                    onClick={() => toast.success("学者影响力报告已生成")}
                  >
