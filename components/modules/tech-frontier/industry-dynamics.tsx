...
import type { IndustryNews } from "@/lib/types/tech-frontier";
import { mockIndustryNews } from "@/lib/mock-data/tech-frontier";
...
export default function IndustryDynamics() {
  ...
  const newsCount = mockIndustryNews.length;
  const investCount = mockIndustryNews.filter(
    (n) => n.type === "投融资" || n.type === "收购",
  ).length;
  const policyCount = mockIndustryNews.filter((n) => n.type === "政策").length;
          ...
          {selectedNews && (
            <>
              <SheetHeader>
                ...
                <SheetDescription>
                  来源: {selectedNews.source} · {selectedNews.date} · 影响评估:{" "}
                  {selectedNews.impact}
                </SheetDescription>
              </SheetHeader>
                ...
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    <Badge
                      ...
                      className={cn(
                        "text-xs",
                        typeConfig[selectedNews.type].bg,
                        typeConfig[selectedNews.type].color,
                      )}
                    >
                      {selectedNews.type}
                    </Badge>
                    <Badge
                      ...
                      className={cn(
                        "text-xs",
                        impactConfig[selectedNews.impact].bg,
                        impactConfig[selectedNews.impact].color,
                      )}
                    >
